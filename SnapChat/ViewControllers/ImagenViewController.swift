...
class ImagenViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        imagenesFolder.child("imagenes.jpg").putData(imagenData!, metadata: nil) {
            (metadata, error) in
            if error != nil {
                print("Ocurrio un error al subir la imaen: \(error)")
            }
        }
   ...
   func mostrarAlerta(titulo: String, mensaje:String, accion: String) {
    ...
    @IBAction func elegirContactoTapped(_ sender: Any) {
        ...
        let cargarImagen = imagenesFolder.child("\(NSUUID().uuidString).jpg").putData(imagenData!, metadata: nil) {(metadata, error) in
            if error != nil {
                self.mostrarAlerta(titulo: "Error", mensaje: "Se produjo un error al subir la imagen. Verifique su conexion ..", accion: "Aceptar")
